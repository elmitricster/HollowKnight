...
namespace ya
{
	...
	void CollisionManager::ColliderCollision(Collider2D* left, Collider2D* right)
	{
		...
		{
			// 최초 충돌중 Enter
			if (iter->second == false) // 충돌 중인 상태 (Stay)
			{
				if (left->IsTrigger())
		...
		{
			if (iter->second == true) // 충돌 중인 상태 (Exit)
			{
				if (left->IsTrigger())
	...
	bool CollisionManager::Intersect(Collider2D* left, Collider2D* right)
	{
		...
		{
			// 숙제 Circle vs Circle
			Vector2 leftPos = { left->GetPosition().x, left->GetPosition().y };
			Vector2 leftSize = left->GetSize();
			Vector2 rightPos = { right->GetPosition().x, right->GetPosition().y };

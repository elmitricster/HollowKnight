...
#pragma endregion
	...
	void LoadBuffer()
	{
		...
		constantBuffers[(UINT)eCBType::FadeInAndOut] = new ConstantBuffer(eCBType::FadeInAndOut);
		constantBuffers[(UINT)eCBType::FadeInAndOut]->Create(sizeof(FadeInAndOutCB));
	...
	void LoadShader()
	{
		...
		fadeShader->Create(eShaderStage::VS, L"FadeInAndOutVS.hlsl", "main");
		fadeShader->Create(eShaderStage::PS, L"FadeInAndOutPS.hlsl", "main");
	...
	void LoadTexture()
	{
		Resources::Load<Texture>(L"KokomiTexture", L"kokomi.png");
	...
	{
		// Default
		std::shared_ptr<Texture> texture = Resources::Find<Texture>(L"KokomiTexture");
		std::shared_ptr<Shader> shader = Resources::Find<Shader>(L"RectShader");

...
namespace ya
{
	...
	void PlayScene::Initialize()
	{
		...
		// 플레이어 코코미 RECT
		{
			...
			std::shared_ptr<Material> material = Resources::Find<Material>(L"KnightMaterial");
		...
		//// 몬스터 코코미 RECT
		{
			Monster* obj = object::Instantiate<Monster>(eLayerType::Monster, this);
			obj->SetName(L"KOKOMI2");
			Transform* tr = obj->GetComponent<Transform>();
			...
			SpriteRenderer* mr = obj->AddComponent<SpriteRenderer>();
			std::shared_ptr<Material> material = Resources::Find<Material>(L"BossMaterial");
			mr->SetMaterial(material);
			std::shared_ptr<Mesh> mesh = Resources::Find<Mesh>(L"RectMesh");
			mr->SetMesh(mesh);
			//object::DontDestroyOnLoad(obj);
